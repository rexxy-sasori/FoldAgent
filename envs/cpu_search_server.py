#!/usr/bin/env python3
"""
CPU-optimized search server
python cpu_search_server.py --host 0.0.0.0 --port 8000
"""
...
def validate_model_files(model_name: str) -> bool:
    """Validate that model files are properly downloaded"""
    try:
        from transformers import AutoConfig
        config = AutoConfig.from_pretrained(model_name, local_files_only=True)
        logger.info(f"Model {model_name} files validated successfully")
        return True
...
def cpu_optimized_worker(worker_id: int, batch_queue: mp.Queue, result_queue: mp.Queue,
                        corpus_data: Dict):
    """CPU-optimized worker for search operations"""
    try:
        # Set CPU device (disable GPU)
        os.environ['CUDA_VISIBLE_DEVICES'] = ''  # Disable CUDA for this worker
        
        # Configure PyTorch for CPU optimization
        torch.set_num_threads(int(os.getenv("CPU_THREADS_PER_WORKER", "4")))
        torch.set_num_interop_threads(int(os.getenv("CPU_INTEROP_THREADS", "2")))
        
        # Load model on CPU with optimizations and retry logic
        logger.info(f"Loading model {MODEL_NAME} on CPU for worker {worker_id}")
        tokenizer, model = load_model_with_retry(MODEL_NAME, worker_id)
        
        # Load corpus embeddings on CPU
        corpus_embeddings = corpus_data['embeddings'].cpu().float()  # Ensure CPU and float32
        corpus_docids = corpus_data['docids']
        task_description = 'Given a web search query, retrieve relevant passages that answer the query'

        logger.info(f"CPU Worker {worker_id}: Ready")
        ...
        while True:
            try:
                ...
                cpu_process_batch(batch, tokenizer, model, 
        ...
        logger.error(f"CPU Worker {worker_id} init error: {e}")


def cpu_process_batch(batch: SearchBatch, tokenizer, model,
                     corpus_embeddings, corpus_docids, task_description, result_queue):
    ...
    try:
        ...
        # No need to move to device since we're on CPU
        ...
        with torch.no_grad():
            ...
            # Compute similarities on CPU (using torch.mm which works on CPU)
            similarities = torch.mm(query_embeddings, corpus_embeddings.T)

        # Process results
        for i, request in enumerate(batch.requests):
            # Use CPU operations for top-k
            scores, indices = torch.topk(similarities[i], k=min(request.k, len(corpus_docids)))

            results = []
            for score, idx in zip(scores.tolist(), indices.tolist()):  # Convert to Python lists
...
class CPUSearchServer:
    def __init__(self, num_workers: int = 4, max_batch_size: int = 128, batch_timeout: float = 0.01):
        
        # Validate model files before starting workers
        logger.info("Validating model files...")
        if not validate_model_files(MODEL_NAME):
            logger.error(f"Model {MODEL_NAME} files are not properly downloaded!")
            raise RuntimeError(f"Model {MODEL_NAME} validation failed. Please ensure the model is properly downloaded.")
        
        # Optimized for CPU: Smaller queues and different timeouts
        self.request_queue = mp.Queue(maxsize=5000)  # Smaller queue for CPU
        self.batch_queue = mp.Queue(maxsize=500)     # Smaller batch queue
        self.result_queue = mp.Queue(maxsize=5000)   # Smaller result queue
        ...
        # Start CPU-optimized workers
        self.workers = []
        for worker_id in range(num_workers):
            worker = mp.Process(target=cpu_optimized_worker,
                                args=(worker_id, self.batch_queue, self.result_queue, self.corpus_data))
            worker.start()
            self.workers.append(worker)

        logger.info(f"🚀 Started {num_workers} CPU workers with max_batch_size={max_batch_size}")
    ...
    async def search(self, query: str, k: int = 20) -> Dict:
        ...
        try:
            ...
            result = await asyncio.wait_for(future, timeout=120.0)  # Increased timeout for CPU
    ...
    def shutdown(self):
        """Clean shutdown"""
        logger.info("Shutting down CPU search server...")
...
# Global server
search_server: CPUSearchServer = None
...
async def startup_event():
    ...
    num_workers = int(os.getenv("CPU_WORKERS", "4"))
    max_batch_size = int(os.getenv("MAX_BATCH_SIZE", "128"))  # Smaller for CPU
    batch_timeout = float(os.getenv("BATCH_TIMEOUT", "0.01"))  # Different timeout for CPU

    logger.info(f"🚀 Starting CPU search server: {num_workers} workers, batch={max_batch_size}, timeout={batch_timeout}s")

    search_server = CPUSearchServer(
        num_workers=num_workers, max_batch_size=max_batch_size, batch_timeout=batch_timeout)
...
async def get_prometheus_metrics():
    ...
    if stats['request_count'] > 0:
        metrics.append(f"search_server_avg_response_time {stats['avg_time']}")
    else:
        metrics.append(f"search_server_avg_response_time 0.0")
    metrics.append(f"search_server_max_response_time {stats['max_time']}")

    return PlainTextResponse("\n".join(metrics))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='CPU-optimized Search Server')
    ...
    parser.add_argument('--corpus', type=str, default=CORPUS_DATASET, help='Corpus dataset')
    parser.add_argument('--corpus-embedding-dataset', type=str, default=CORPUS_EMBEDDING_DATASET, help='Corpus embedding dataset')
    ...
    if args.corpus != CORPUS_DATASET:
        CORPUS_DATASET = args.corpus
    if args.corpus_embedding_dataset != CORPUS_EMBEDDING_DATASET:
        CORPUS_EMBEDDING_DATASET = args.corpus_embedding_dataset
        
    # Set environment variables for configuration
    os.environ["CPU_WORKERS"] = str(args.workers)
